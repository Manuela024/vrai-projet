...
// // src/components/admin/UserManagement.jsx - VERSION FINALE POUR VOTRE API DJANGO
...
//     try {
//       // Utilisez vos identifiants admin Django
...
//       }
//     } catch (err) {
//       console.warn('⚠️ Impossible d\'obtenir le token JWT:', err.message);
//       return null;
//     }
//   };

//   // 🎯 Charger les utilisateurs depuis VOTRE API Django existante
...
//       // D'abord obtenir un token si nécessaire
//       let token = authToken;
//       if (!token) {
...
//       // ✅ VOTRE API RETOURNE: { status: 'success', count: X, users: [...] }
//       let usersArray = [];
...
//       // 🎯 TRAITEMENT DES DONNÉES DE VOTRE BASE DJANGO
//       const processedUsers = usersArray.map(user => {
//         // Récupérer les dates correctement
//         let dateJoined = null;
//         let lastLogin = null;
...
//         // IMPORTANT: Vérifiez si ces champs existent dans votre réponse
//         const isSuperuser = user.is_superuser || user.is_superuser === true;
//         const isStaff = user.is_staff || user.is_staff === true || isSuperuser;
...
//           // 🎯 CHAMPS CRITIQUES - Vérifiez qu'ils existent dans votre réponse API
...
//           // Champs supplémentaires (si présents dans votre API)
//           projects_count: user.projects_count || 0,
//           profile: user.profile || null
//         };
//       });
      
//       console.log(`✅ ${processedUsers.length} utilisateurs chargés depuis Django`);
//       console.log('👑 Super-admins détectés:', processedUsers.filter(u => u.is_superuser).map(u => u.username));
...
//         errorMessage = 'Serveur Django non accessible. Vérifiez que le serveur est en cours d\'exécution.';
//       } else if (err.response) {
//         errorMessage = `Erreur API (${err.response.status}): ${err.response.data?.message || err.response.statusText}`;
//         if (err.response.status === 401) {
//           errorMessage += '. Token JWT peut-être expiré.';
//         }
//       } else if (err.request) {
//         errorMessage = 'Pas de réponse du serveur Django. Vérifiez l\'URL.';
//       }
...
//   // 🎯 Calcul des statistiques BASÉES SUR LA BASE DE DONNÉES
//   const stats = {
//     total: users.length,
//     active: users.filter(u => u.is_active).length,
//     staff: users.filter(u => u.is_staff).length,
//     super: users.filter(u => u.is_superuser).length, // DIRECTEMENT DE LA BD
//     regular: users.filter(u => !u.is_staff && !u.is_superuser).length, // DIRECTEMENT DE LA BD
//     withLogin: users.filter(u => u.last_login).length,
//     recentLogin: users.filter(u => u.last_login && 
//       (new Date() - new Date(u.last_login)) < 7 * 24 * 60 * 60 * 1000).length,
...
//         <p className="text-sm text-gray-500">Connexion à l'API Django</p>
//         <p className="text-xs text-gray-400 mt-1">Endpoint: /api/users/all/</p>
//       </div>
//     );
//   }
...
//                 <div className="flex-1">
//                   <p className="text-red-700 font-medium mb-1">Impossible de charger les données</p>
//                   <p className="text-red-600 text-sm">{error}</p>
//                   <div className="mt-3 text-sm">
//                     <p className="text-red-800 font-medium mb-1">Vérifiez que :</p>
//                     <ul className="text-red-700 space-y-1">
//                       <li className="flex items-center gap-2">
//                         <div className="w-1 h-1 bg-red-500 rounded-full"></div>
//                         Le serveur Django est en cours d'exécution (<code>python manage.py runserver</code>)
//                       </li>
//                       <li className="flex items-center gap-2">
//                         <div className="w-1 h-1 bg-red-500 rounded-full"></div>
//                         L'endpoint <code className="bg-red-100 px-1 rounded">/api/users/all/</code> est accessible
//                       </li>
//                       <li className="flex items-center gap-2">
//                         <div className="w-1 h-1 bg-red-500 rounded-full"></div>
...
//                 {users.filter(u => u.is_superuser).slice(0, 3).map(u => u.username).join(', ')}
//                 {stats.super > 3 && ` et ${stats.super - 3} autres`}
//               </div>
//             </div>
//             <div className="text-center p-4 bg-white rounded-lg border">
...
//               <div className="text-xs text-gray-500 mt-2">
//                 {stats.regular} simploniens actifs
//               </div>
//             </div>
//             <div className="text-center p-4 bg-white rounded-lg border">
...
//           <div className="mt-3 text-sm text-gray-600">
//             {filteredUsers.length} utilisateur(s) trouvé(s) sur {stats.total}
//             {roleFilter === 'superadmin' && stats.super > 0 && (
//               <span className="ml-2 text-red-600 font-medium">
//                 ({stats.super} super-admin{stats.super > 1 ? 's' : ''})
//               </span>
//             )}
//             {roleFilter === 'user' && stats.regular > 0 && (
...
//             <div>
//               <h3 className="font-medium text-gray-900 mb-3">Source des données</h3>
...
//                     Endpoint API
//                   </span>
//                   <code className="text-xs bg-blue-50 px-2 py-1 rounded">{USERS_API}</code>
//                 </div>
//                 <div className="flex items-center justify-between p-2 bg-white rounded border">
//                   <span>Statut</span>
//                   <span className={`font-medium ${error ? 'text-red-600' : 'text-green-600'}`}>
//                     {error ? 'Erreur de connexion' : 'Connecté'}
//                   </span>
//                 </div>
//                 <div className="flex items-center justify-between p-2 bg-white rounded border">
//                   <span>Utilisateurs chargés</span>
...
//             <div>
//               <h3 className="font-medium text-gray-900 mb-3">Rôles détectés</h3>
//               <div className="space-y-2 text-sm text-gray-600">
//                 <div className="flex items-center justify-between p-2 bg-white rounded border">
//                   <span className="flex items-center gap-2">
...
//                     Admins staff
//                   </span>
//                   <span className="font-medium text-purple-600">{stats.adminRegular}</span>
//                 </div>
//                 <div className="flex items-center justify-between p-2 bg-white rounded border">
//                   <span className="flex items-center gap-2">
...
//             <p className="font-medium text-gray-700 mb-1">ℹ️ Informations techniques</p>
//             <p>Toutes les données proviennent directement de votre API Django. Aucune donnée de test n'est utilisée.</p>
//             <p className="mt-1">Si les rôles ne s'affichent pas correctement, vérifiez que votre API retourne bien les champs <code>is_superuser</code> et <code>is_staff</code>.</p>
//           </div>
//         </div>
//       </div>
//     </div>
