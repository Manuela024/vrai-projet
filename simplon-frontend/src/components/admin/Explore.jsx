...
// // src/components/admin/Explore.jsx - VERSION SIMPLIFIÉE POUR PROJETDETAIL
// import React, { useState, useEffect } from 'react';
// import { useNavigate } from 'react-router-dom';
// import authService from '../../services/auth';
...
//     try {
//       const fallbackEndpoint = 'http://localhost:8000/api/projects/';
//       const response = await axios.get(fallbackEndpoint, { timeout: 5000 });
      
//       if (response.status === 200) {
//         console.log('✅ Connexion Django réussie (fallback)!');
//         return {
//           connected: true,
//           data: response.data,
...
//       }
//     } catch (fallbackError) {
//       console.log('❌ Connexion fallback échouée:', fallbackError.message);
//     }
    
//     return { connected: false, data: null, endpoint: null };
//   };

//   // ✅ CHARGER LES PROJETS DEPUIS DJANGO
//   const fetchProjectsFromDjango = async () => {
//     try {
//       console.log('📡 Tentative de chargement depuis Django...');
      
//       const connection = await testDjangoConnection();
//       setConnectionTested(true);
      
//       if (!connection.connected) {
//         console.log('⚠️ API non disponible, activation mode démo');
//         setIsDemoMode(true);
//         return getDemoProjects();
//       }
...
//       // Extraire les projets de différentes structures de réponse
//       let projectsData = [];
      
//       if (apiData.projects && Array.isArray(apiData.projects)) {
//         projectsData = apiData.projects;
//       } else if (Array.isArray(apiData)) {
//         projectsData = apiData;
//       } else if (apiData.results && Array.isArray(apiData.results)) {
//         projectsData = apiData.results;
//       } else if (apiData.items && Array.isArray(apiData.items)) {
//         projectsData = apiData.items;
//       }
      
//       console.log(`✅ ${projectsData.length} projets extraits`);
      
//       if (projectsData.length === 0) {
//         console.log('⚠️ Aucun projet dans la réponse API');
//         setIsDemoMode(true);
//         return getDemoProjects();
//       }
...
//       // Transformer les données pour correspondre à ProjectDetail
//       const transformedProjects = projectsData.map((project, index) => {
//         // Image du projet
//         let imageUrl = '';
//         if (project.image) {
...
//         // Statut
//         const status = project.status || 'draft';
//         const isApproved = project.is_approved || status === 'approved';
//         const isPending = project.is_pending || status === 'pending';
//         const isRejected = project.is_rejected || status === 'rejected';
//         const isDraft = project.is_draft || status === 'draft';
        
//         // Technologies (format compatible)
//         let technologies = '';
//         if (project.technologies) {
...
//           status: status,
//           cohort: project.cohort || 'Promotion non spécifiée',
//           github_url: project.github_url || '',
//           demo_url: project.demo_url || '',
//           image: imageUrl,
//           created_at: project.created_at || new Date().toISOString(),
//           updated_at: project.updated_at || new Date().toISOString(),
//           author_name: authorName || 'Auteur inconnu',
//           author_email: authorEmail || '',
//           author_id: project.author_id || project.author?.id || null,
          
//           // Flags de statut (compatibles avec ProjectDetail)
//           is_approved: isApproved,
...
//   // ✅ DONNÉES DE DÉMONSTRATION (compatibles avec ProjectDetail)
//   const getDemoProjects = () => {
//     return [
//       {
//         id: 1,
...
//         full_description: `# Application Météo React Native\n\n## Description du projet\nApplication météo développée avec React Native pour iOS et Android. Utilise l'API OpenWeatherMap pour les données en temps réel et propose des prévisions sur 7 jours.\n\n## Fonctionnalités\n• Données en temps réel\n• Prévisions 7 jours\n• Notifications météo\n• Mode hors ligne\n• Interface intuitive`,
...
//       // Charger les projets
//       const projectsData = await fetchProjectsFromDjango();
//       setProjects(projectsData);
      
//       // Message d'information
//       if (isDemoMode) {
//         setError('⚠️ Mode démo - Affichage de données de démonstration');
//       } else {
//         setError(`✅ ${projectsData.length} projets chargés depuis Django`);
//       }
...
//       setProjects(demoProjects);
//       setIsDemoMode(true);
//       setError('❌ Erreur de connexion à l\'API - Mode démo activé');
...
//   // ✅ NAVIGUER VERS LA PAGE DE DÉTAIL (avec toutes les données nécessaires)
//   const handleViewDetails = (project) => {
//     // Stocker les données complètes pour ProjectDetail
//     const projectWithFullData = {
//       ...project,
//       // Assurer que toutes les données nécessaires sont présentes
//       full_description: project.full_description || project.description || 'Aucune description détaillée disponible',
...
//     // Naviguer vers ProjectDetail avec toutes les données
//     navigate(`/admin/projects/${project.id}`, { 
...
//         const titleMatch = project.title?.toLowerCase().includes(searchLower) || false;
//         const descMatch = project.description?.toLowerCase().includes(searchLower) || false;
//         const authorMatch = project.author_name?.toLowerCase().includes(searchLower) || false;
//         const techMatch = project.technologies?.toLowerCase().includes(searchLower) || false;
//         const cohortMatch = project.cohort?.toLowerCase().includes(searchLower) || false;
...
//     if (categoryFilter !== 'all') {
//       filtered = filtered.filter(project => 
//         (project.category || 'other').toLowerCase() === categoryFilter
//       );
//     }
...
//   // ✅ BADGE DE STATUT (compatible avec ProjectDetail)
//   const getStatusBadge = (project) => {
//     const status = project.status?.toLowerCase() || 'draft';
//     const isApproved = project.is_approved || status === 'approved';
//     const isRejected = project.is_rejected || status === 'rejected';
...
//   // ✅ FILTRAGE LORSQUE LES DONNÉES OU FILTRES CHANGENT
//   useEffect(() => {
//     filterAndSortProjects();
//   }, [projects, searchTerm, categoryFilter, statusFilter, sortBy]);
...
//           {connectionTested ? 'Chargement des projets...' : 'Connexion à l\'API en cours...'}
//         </p>
//         <p className="text-gray-600 text-center max-w-md">
//           {isDemoMode ? 'Chargement des données démo' : 'Récupération depuis Django'}
//         </p>
//       </div>
//     );
//   }
...
//             className="px-3 py-1.5 bg-[#001F3F] text-white rounded-full text-sm hover:bg-[#003265] transition-colors flex items-center gap-1"
//           >
...
//                 className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E30613]/20 focus:border-[#E30613]"
//               />
//             </div>
//           </div>
...
//               value={categoryFilter}
//               onChange={(e) => setCategoryFilter(e.target.value)}
//               className="w-full px-3 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E30613]/20 focus:border-[#E30613]"
//             >
//               {categories.map(cat => (
//                 <option key={cat.value} value={cat.value}>{cat.label}</option>
//               ))}
...
//                 value={sortBy}
//                 onChange={(e) => setSortBy(e.target.value)}
//                 className="flex-1 px-3 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E30613]/20 focus:border-[#E30613]"
//               >
//                 {sortOptions.map(option => (
//                   <option key={option.value} value={option.value}>{option.label}</option>
//                 ))}
...
//             {filteredProjects.map(project => {
//               const projectImage = getProjectImage(project);
//               const authorInitial = project.author_name?.charAt(0).toUpperCase() || 'A';
...
//                   {/* Contenu */}
//                   <div className="flex-1 p-4">
//                     <h3 className="text-lg font-bold text-[#001F3F] mb-2 line-clamp-1">
//                       {project.title}
//                     </h3>
...
//                       <button
//                         onClick={() => handleViewDetails(project)}
//                         className="px-4 py-2 bg-[#001F3F] text-white rounded-lg text-sm font-medium flex items-center gap-2 hover:bg-[#003265] transition-colors"
//                       >
...
//         // URL du fichier ZIP (nouveau champ)
//         let zipFileUrl = '';
//         if (project.zip_file) {
...
//               zipFileUrl = `http://127.0.0.1:8000${project.zip_file}`;
//             } else if (project.zip_file.includes('.zip')) {
//               zipFileUrl = `http://127.0.0.1:8000/media/${project.zip_file}`;
//             }
//           }
//         }
...
//           github_url: project.github_url || '',
//           demo_url: project.demo_url || '',
//           zip_file: zipFileUrl, // Nouveau champ
//           image: imageUrl,
//           created_at: project.created_at || new Date().toISOString(),
//           updated_at: project.updated_at || new Date().toISOString(),
//           author_name: authorName || 'Auteur inconnu',
...
//         github_url: "https://github.com/example/portfolio-react",
//         demo_url: "https://portfolio-react-demo.netlify.app",
//         zip_file: "https://example.com/downloads/portfolio-react.zip", // Exemple
//         image: "https://images.unsplash.com/photo-1551650975-87deedd944c3?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
//         created_at: "2024-03-15T10:30:00Z",
//         updated_at: "2024-03-20T14:30:00Z",
//         author_name: "Léa Martin",
...
//         github_url: "https://github.com/example/ecommerce-app",
//         demo_url: "https://ecommerce-demo.vercel.app",
//         zip_file: "https://example.com/downloads/ecommerce-app.zip", // Exemple
//         image: "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
//         created_at: "2024-04-20T14:15:00Z",
//         updated_at: "2024-04-25T09:20:00Z",
//         author_name: "Mohamed Ali",
...
//         github_url: "https://github.com/example/weather-app",
//         demo_url: "",
//         zip_file: "https://example.com/downloads/weather-app.zip", // Exemple
//         image: "https://images.unsplash.com/photo-1592210454359-9043f067919b?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
//         created_at: "2024-01-05T11:20:00Z",
//         updated_at: "2024-01-10T14:45:00Z",
//         author_name: "Thomas Bernard",
...
// // src/components/admin/Explore.jsx - VERSION COMPLÈTE
// import React, { useState, useEffect } from 'react';
// import { useNavigate } from 'react-router-dom';
// import authService from '../../services/auth';
// import axios from 'axios';
...
// src/components/admin/Explore.jsx - VERSION COMPLÈTE CORRIGÉE
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import authService from '../../services/auth';
import axios from 'axios';
...
const Explore = () => {
  ...
  // Configuration de l'API - VOTRE API FONCTIONNE SUR LOCALHOST:8000
  ...
  // ✅ TESTER LA CONNEXION À L'API - VERSION SIMPLIFIÉE ET FIABLE
  const testDjangoConnection = async () => {
    console.log('🔍 Test de connexion à Django...');
    
    try {
      // Essayer avec fetch (plus simple)
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('✅ Connexion Django réussie! Status:', response.status);
        console.log('📊 Structure des données:', {
          status: data.status,
          count: data.count,
          hasProjects: !!(data.projects && Array.isArray(data.projects)),
        ...
        return { connected: false, data: null, endpoint: null, statusCode: response.status };
      }
      
    } catch (error) {
      console.log('❌ Erreur de connexion:', error.message);
      return { connected: false, data: null, endpoint: null, statusCode: 0 };
    }
  };

  // ✅ CHARGER LES PROJETS DEPUIS DJANGO - VERSION CORRIGÉE
  const fetchProjectsFromDjango = async () => {
    try {
      ...
      if (!connection.connected) {
        console.log('⚠️ API non disponible, activation mode démo');
        setIsDemoMode(true);
        setError('❌ Impossible de se connecter à l\'API - Mode démo activé');
        return getDemoProjects();
      }
      ...
      // ✅ EXTRACTION DES PROJETS - GESTION DE TOUS LES FORMATS
      let projectsData = [];
      
      // FORMAT PRINCIPAL: {"status":"success","count":12,"projects":[...]}
      if (apiData.projects && Array.isArray(apiData.projects)) {
        console.log('📦 Format détecté: apiData.projects');
        projectsData = apiData.projects;
      }
      ...
      // FORMAT PAGINATION: {"results":[...]}
        ...
        console.log('📦 Format détecté: apiData.results');
        projectsData = apiData.results;
      }
      // FORMAT ITEMS: {"items":[...]}
      else if (apiData.items && Array.isArray(apiData.items)) {
        console.log('📦 Format détecté: apiData.items');
        projectsData = apiData.items;
      }
      // FORMAT STATS: {"projects":{"projects":[...]}}
      else if (apiData.projects && apiData.projects.projects && Array.isArray(apiData.projects.projects)) {
        console.log('📦 Format détecté: apiData.projects.projects');
        projectsData = apiData.projects.projects;
      }
      
      console.log(`✅ ${projectsData.length} projets extraits (sur ${apiData.count} annoncés)`);
      
      // DEBUG: Afficher les 3 premiers projets
      if (projectsData.length > 0) {
        console.log('🔍 Structure du premier projet:', {
          id: projectsData[0].id,
          title: projectsData[0].title,
          description: projectsData[0].description?.substring(0, 50) + '...',
          keys: Object.keys(projectsData[0])
        });
      }
      
      if (projectsData.length === 0) {
        console.log('⚠️ Aucun projet extrait, structure complète:', apiData);
        setIsDemoMode(true);
        setError('⚠️ Aucun projet trouvé dans la réponse API - Mode démo activé');
        return getDemoProjects();
      }
      ...
      const transformedProjects = projectsData.map((project, index) => {
        // TITRE
        const title = project.title || `Projet ${project.id || index + 1}`;
        ...
        if (project.image) {
          if (typeof project.image === 'string') {
            if (project.image.startsWith('http')) {
              ...
              imageUrl = `${API_BASE}${project.image}`;
            } else if (project.image.includes('.')) {
              imageUrl = `${API_BASE}/media/${project.image}`;
            } else if (project.image) {
              imageUrl = project.image; // URL déjà complète
            }
          }
        }
        ...
        if (project.zip_file) {
          if (typeof project.zip_file === 'string') {
            if (project.zip_file.startsWith('http')) {
              ...
              zipFileUrl = `${API_BASE}${project.zip_file}`;
            } else if (project.zip_file.includes('.zip')) {
              zipFileUrl = `${API_BASE}/media/${project.zip_file}`;
            } else {
              zipFileUrl = project.zip_file;
            }
          }
        ...
        if (project.author_name) {
          ...
          if (typeof project.author === 'object') {
            if (project.author.first_name || project.author.last_name) {
              authorName = `${project.author.first_name || ''} ${project.author.last_name || ''}`.trim();
            } else if (project.author.username) {
              authorName = project.author.username;
            } else if (project.author.name) {
              authorName = project.author.name;
        ...
        // COHORTE
        const cohort = project.cohort || 'Non spécifié';
        
        // ID (CRITIQUE)
        const projectId = project.id || `db-${index}`;
        ...
        return {
          ...
          full_description: description, // Pour compatibilité avec ProjectDetail
      ...
      console.error('❌ ERREUR LORS DU CHARGEMENT:', error);
      setIsDemoMode(true);
      setError(`❌ Erreur de chargement: ${error.message} - Mode démo activé`);
      return getDemoProjects();
    }
  };
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      setProjects(projectsData);
      console.log(`📊 ${projectsData.length} projets mis dans l'état`);
      
      // Mettre à jour le message d'erreur/succès
      if (isDemoMode) {
        setError('⚠️ Mode démo - Affichage de données de démonstration');
      } else {
        setError(`✅ ${projectsData.length} projets chargés depuis l'API Django`);
      }
  ...
  const handleDownloadZip = (project) => {
    ...
    console.log('📦 Téléchargement du ZIP:', project.zip_file);
  ...
  // ✅ NAVIGUER VERS LA PAGE DE DÉTAIL
  const handleViewDetails = (project) => {
    console.log('🔍 Navigation vers détail du projet:', project.id, project.title);
    
    // Préparer les données pour ProjectDetail
    const projectWithFullData = {
      ...project,
      // Assurer que toutes les données nécessaires sont présentes
      full_description: project.full_description || project.description || 'Aucune description détaillée disponible',
  ...
  // ✅ CHARGEMENT INITIAL
  useEffect(() => {
    console.log('🚀 Explore.jsx - Initialisation');
    fetchProjects();
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[60vh] p-8">
        <div className="animate-spin rounded-full h-20 w-20 border-t-4 border-b-4 border-[#E30613] mb-8"></div>
        ...
        {apiStatus && (
          <div className="bg-gray-100 p-4 rounded-lg max-w-md">
            <p className="text-sm font-medium text-gray-700 mb-2">Statut API:</p>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <span className="text-gray-600">Connecté:</span>
              <span className={apiStatus.connected ? 'text-green-600 font-medium' : 'text-red-600'}>
                {apiStatus.connected ? '✅ Oui' : '❌ Non'}
              </span>
              
              <span className="text-gray-600">Endpoint:</span>
              <span className="text-blue-600 truncate">{apiStatus.endpoint || 'Non testé'}</span>
              
              <span className="text-gray-600">Status Code:</span>
              <span className={apiStatus.statusCode === 200 ? 'text-green-600' : 'text-red-600'}>
                {apiStatus.statusCode || 'N/A'}
              </span>
            </div>
          </div>
        )}
              ...
              <div>
                ...
                {apiStatus && !isDemoMode && (
                  <p className="text-xs text-gray-600 mt-1">
                    Endpoint: {apiStatus.endpoint} • Status: {apiStatus.statusCode}
                  </p>
                )}
              </div>
            </div>
