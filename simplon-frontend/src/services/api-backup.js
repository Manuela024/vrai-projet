...
// src/services/api.js - VERSION OPTIMISÉE POUR TOUS LES RÔLES
import axios from 'axios';

// Configuration
const API_BASE_URL = 'http://localhost:8000';

console.log(`🔧 API Django configurée: ${API_BASE_URL}`);
...
// Intercepteur request pour ajouter le token
api.interceptors.request.use(
  (config) => {
...
// Intercepteur response pour gérer les erreurs
api.interceptors.response.use(
  (response) => {
  ...
  (error) => {
    ...
    if (response) {
      console.error(`❌ Erreur ${response.status}: ${response.config.url}`);
...
// Récupérer les projets avec filtrage par rôle
export const fetchProjectsByRole = async (role = 'user') => {
  console.log(`📡 Récupération projets pour rôle: ${role}`);
  
  let endpoint;
  
  switch (role) {
    case 'admin':
      endpoint = '/api/projects/projects-with-users/';
      break;
    case 'all':
      endpoint = '/api/projects/projects/all/';
      break;
    case 'user':
    default:
  ...
  try {
    ...
    if (response.data.projects && Array.isArray(response.data.projects)) {
      projects = response.data.projects;
    } else if (response.data.users_with_projects) {
      // Format admin: users_with_projects
      response.data.users_with_projects.forEach(userWithProjects => {
        if (userWithProjects.projects) {
          projects = projects.concat(userWithProjects.projects.map(p => ({
            ...p,
            user_info: userWithProjects.user
          })));
        }
      });
    } else if (Array.isArray(response.data)) {
    ...
    console.log(`✅ ${projects.length} projet(s) récupéré(s) pour ${role}`);
    return projects;
    
  } catch (error) {
    console.error(`❌ Erreur fetchProjectsByRole (${role}):`, error);
    throw error;
  }
};
...
export const testApiConnection = async () => {
  const endpoints = [
    { name: 'API Racine', url: '/api/projects/' },
    { name: 'Liste Projets', url: '/api/projects/projects/' },
    { name: 'Statistiques', url: '/api/projects/stats/' },
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      results.push({
        ...
        data: response.data ? '✓ Données' : '✗ Vide'
      });
    } catch (error) {
      results.push({
        name: endpoint.name,
...
export const apiService = {
  ...
  async checkHealth() {
    try {
      const response = await api.get('/api/projects/stats/', { timeout: 3000 });
      return {
        healthy: true,
...
export default {
  ...
  // Méthodes spécifiques exportées individuellement
  get: apiService.get,
