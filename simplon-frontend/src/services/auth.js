...
// src/services/auth.js - VERSION CORRIGÉE COMPLÈTE
const API_URL = 'http://localhost:8000';

// Fonction pour unifier les clés d'authentification
const migrateAndUnifyAuthKeys = () => {
  // Migrer vers les clés standardisées
  const oldToNew = {
    'access_token': 'simplon_access_token',
    'user': 'simplon_user',
    'user_id': 'simplon_user_id',
    'user_role': 'simplon_user_role'
  };
  ...
  // S'assurer que les clés simplon_* existent
  const userStr = localStorage.getItem('simplon_user') || localStorage.getItem('user');
  if (userStr && !localStorage.getItem('simplon_user')) {
    localStorage.setItem('simplon_user', userStr);
  }
  
  const token = localStorage.getItem('simplon_access_token') || localStorage.getItem('access_token');
  if (token && !localStorage.getItem('simplon_access_token')) {
    localStorage.setItem('simplon_access_token', token);
  }
};

// Appeler la migration au chargement
migrateAndUnifyAuthKeys();
...
const authService = {
  ...
  _storeAuthData(accessToken, userData) {
    // TOUJOURS stocker avec les clés standardisées 'simplon_'
    localStorage.setItem('simplon_access_token', accessToken);
    localStorage.setItem('simplon_user', JSON.stringify(userData));
    localStorage.setItem('simplon_user_id', userData.id?.toString() || '');
    localStorage.setItem('simplon_user_role', userData.role || '');
  ...
  getAccessToken() {
    // Chercher d'abord les clés standardisées
    return localStorage.getItem('simplon_access_token') || 
  ...
  isAdmin() {
    const user = this.getCurrentUser();
    if (!user) {
      console.log('🔍 Pas d\'utilisateur pour vérifier admin');
      return false;
    }
    
    // Vérifier plusieurs façons de définir un admin
    const isAdmin = 
      user.is_staff === true ||
      user.is_admin === true ||
      user.isAdmin === true ||
      ...
      (user.first_name && user.first_name.toLowerCase().includes('admin')) ||
      user.id === 1 || // Souvent l'admin a l'ID 1
      false;
  ...
  debugAuth() {
    ...
    console.log('Access Token:', this.getAccessToken() ? 'Oui' : 'Non');
    console.log('Current User:', this.getCurrentUser());
    console.log('Is Authenticated:', this.isAuthenticated());
    console.log('Is Admin:', this.isAdmin());
    console.log('All localStorage:', Object.keys(localStorage).map(key => `${key}: ${localStorage.getItem(key)?.substring(0, 50)}...`));
  }
};
